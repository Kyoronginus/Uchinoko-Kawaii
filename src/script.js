...
async function initializeScene() {
  try {
    ...
    character.physicsBody = physicsManager.addCharacterBody(
    ...
    console.log(
      "🔍 Gathering interactive objects for ProximityInteractionManager:"
    );
    console.log(`  - Signpost models: ${signpostModels.length}`);
    signpostModels.forEach((entry, i) => {
      console.log(
        `    [${i}] ${entry.item.name} (${entry.item.type}) - has URL: ${!!entry
          .item.url}`
      );
    });
    ...
    proximityManager = new ProximityInteractionManager(
      interactiveObjects,
      scene,
      {
...
function animate() {
  ...
  if (character.physicsBody) {
    ...
    const desiredVel = character.getDesiredVelocity
      ? character.getDesiredVelocity()
      : new THREE.Vector3();
    ...
    character.physicsBody.position.y = Math.max(
      character.physicsBody.position.y,
      0.9
    );
  }
